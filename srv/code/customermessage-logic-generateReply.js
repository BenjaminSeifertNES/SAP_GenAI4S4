/**
 * 
 * @On(event = { "Action1" }, entity = "bTPGenAI4S4_BenjaminSeifert3Srv.CustomerMessage")
...
module.exports = async function (request) {
	...
	try {
		...
		customerMessage = await SELECT.one.from('BTPGenAI4S4_BenjaminSeifert3.CustomerMessage').where({ ID });
		if (!customerMessage) {
	...
	if (messageCategory === 'Technical') {
		...
		try {
			...
			relevantFAQs = await SELECT.from('BTPGenAI4S4_BenjaminSeifert3.ProductFAQ')
	...
	try {
		...
		await UPDATE('BTPGenAI4S4_BenjaminSeifert3.CustomerMessage').set({
