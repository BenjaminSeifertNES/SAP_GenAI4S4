/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "bTPGenAI4S4_BenjaminSeifert3Srv.ProductFAQ")
...
module.exports = async function (results, request) {
	...
	try {
		...
		productFAQ = await SELECT.one('BTPGenAI4S4_BenjaminSeifert3.ProductFAQ').where({ ID: productFAQID }).forUpdate();
		if (!productFAQ) {
	...
	try {
		...
		await UPDATE('BTPGenAI4S4_BenjaminSeifert3.ProductFAQ').set({ embedding: embedding }).where({ ID });
